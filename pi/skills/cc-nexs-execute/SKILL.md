...
description: /cc-nexs:execute 的 Pi P2 适配 skill。 支持 preset-standard lean（默认）与 fast 模式，并通过 pi-subagents 运行隔离角色。 Lean 实现入口。按 plan 任务和路径所有权并行实现，不负责 Git mutation。
---
