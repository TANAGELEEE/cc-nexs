...
description: /cc-nexs:approve-deploy 的 Pi P2 适配 skill。 仅允许通过 /cc-nexs:approve-deploy 或 /skill:cc-nexs-approve-deploy 显式调用；不得因普通自然语言请求自动触发。 支持 preset-standard lean（默认）、fast 与 full 模式，并通过 pi-subagents 运行隔离角色。 Approve deploy gate G2 in authoritative progress.json v2, mirror the human-readable view, then resume via /cc-nexs:run.
