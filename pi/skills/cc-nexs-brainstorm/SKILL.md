...
description: /cc-nexs:brainstorm 的 Pi P2 适配 skill。 仅允许通过 /cc-nexs:brainstorm 或 /skill:cc-nexs-brainstorm 显式调用；不得因普通自然语言请求自动触发。 支持 preset-standard lean（默认）与 fast 模式，并通过 pi-subagents 运行隔离角色。 Brainstorming 入口。用 Socratic 对话把 PM 一句话诉求展开成结构化 requirements.md，作为 Planner 的输入。在 /cc-nexs:init 之后、/cc-nexs:run 之前调用。
