---
name: cc-nexs-doctor
description: /cc-nexs:doctor 的 Pi P2 适配 skill。 仅允许通过 /cc-nexs:doctor 或 /skill:cc-nexs-doctor 显式调用；不得因普通自然语言请求自动触发。 支持 preset-standard lean（默认）、fast 与 full 模式，并通过 pi-subagents 运行隔离角色。 Validate workspace repositories, private overlay, and progress.json v2 files without changing project state.
