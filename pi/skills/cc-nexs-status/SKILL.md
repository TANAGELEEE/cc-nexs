---
name: cc-nexs-status
description: /cc-nexs:status 的 Pi P2 适配 skill。 仅允许通过 /cc-nexs:status 或 /skill:cc-nexs-status 显式调用；不得因普通自然语言请求自动触发。 支持 preset-standard lean（默认）与 fast 模式，并通过 pi-subagents 运行隔离角色。 Read-only status snapshot of the active feature pipeline.
