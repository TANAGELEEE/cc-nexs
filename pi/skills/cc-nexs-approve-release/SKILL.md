...
description: /cc-nexs:approve-release 的 Pi P2 适配 skill。 仅允许通过 /cc-nexs:approve-release 或 /skill:cc-nexs-approve-release 显式调用；不得因普通自然语言请求自动触发。 支持 preset-standard lean（默认）与 fast 模式，并通过 pi-subagents 运行隔离角色。 Approve the exact Lean or Hotfix candidate verified in test before base-branch integration.
