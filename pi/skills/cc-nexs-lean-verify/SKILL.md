...
description: /cc-nexs:lean-verify 的 Pi P2 适配 skill。 仅允许通过 /cc-nexs:lean-verify 或 /skill:cc-nexs-lean-verify 显式调用；不得因普通自然语言请求自动触发。 支持 preset-standard lean（默认）、fast 与 full 模式，并通过 pi-subagents 运行隔离角色。 Lean test 环境黑盒验收，直接把 AC 证据汇总到 plan.md。
disable-model-invocation: true
---

# /cc-nexs:lean-verify for Pi

Read and follow `../../../dist/preset-standard/commands/lean-verify.md` as the authoritative command. Treat the text after `/cc-nexs:lean-verify` as its arguments.
