...
description: /cc-nexs:release-test 的 Pi P2 适配 skill。 仅允许通过 /cc-nexs:release-test 或 /skill:cc-nexs-release-test 显式调用；不得因普通自然语言请求自动触发。 支持 preset-standard lean（默认）、fast 与 full 模式，并通过 pi-subagents 运行隔离角色。 Integrate final feature candidates into test, run the project release driver, then hand deployed evidence to the black-box Verifier.
disable-model-invocation: true
---

# /cc-nexs:release-test for Pi

Read and follow `../../../dist/preset-standard/commands/release-test.md` as the authoritative command. Treat the text after `/cc-nexs:release-test` as its arguments.

## Deterministic Test Release Control

Resolve `../../../packages/core/lib/cc-nexs-cli.mjs` relative to this SKILL.md and deliver the exact candidate to the test branch/CI first:
