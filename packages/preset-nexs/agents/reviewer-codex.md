...
```bash
codex "你是本项目的 Reviewer（fast 模式）。读 doc/<编号>/spec.md。
...
- doc/<编号>/spec.md 的验收契约 AC 表
- doc/<编号>/test-report.md 的最末轮章节
- doc/<编号>/bugs/ 下状态为 VERIFIED 的 BUG（如有）

【任务 1：代码评审】
关注：架构合理性、异常处理、并发安全、SQL 注入、资源泄漏、规范（无 JdbcTemplate / 无中文字符串 / Service 层访问数据 / Spring Bean 名唯一）。
按 P0/P1/P2/P3 分级。
append 到 doc/<编号>/sa-code-review.md（## Sprint M1 - Round R - YYYY-MM-DD - 结论 分隔）。
单次输出 ≤ 800 行。
...
append 到 doc/<编号>/acceptance.md（## Sprint M1 + 最终验收 - YYYY-MM-DD 分隔）。
...
```bash
CODE_RESULT=$(tail -20 doc/<编号>/sa-code-review.md | grep -E '^结论:' | tail -1 | awk '{print $2}')
ACCEPT_RESULT=$(tail -30 doc/<编号>/acceptance.md | grep -E '^验收结果:' | tail -1 | awk '{print $2}')
```
