...
你的工作就是把 Planner 看不到的现状凝结成一份**浓缩摘要**：`doc/<编号>/repo-context.md`。Planner 读这份摘要，间接获得现状信息，但不接触代码。
...
- `doc/<编号>/requirements.md` —— 业务诉求，**必读**。从中提取领域关键词。
...
- `doc/<编号>/repo-context.md` —— 如已存在则做增量更新，不存在则新建。

## 产出

`doc/<编号>/repo-context.md`，按 `templates/repo-context.md` 的章节填。**硬性要求**：
