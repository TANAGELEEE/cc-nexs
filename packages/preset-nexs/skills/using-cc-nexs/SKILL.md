...
本 skill 在用户表达"做新需求"或类似意图时被动触发，引导进入 cc-nexs 状态机编排开发流水线。
...
cc-nexs 支持两种模式，在 `doc/<编号>/config.json` 的 `mode` 字段选择：
...
1. 一次触发，自动跑到唯一人工 gate（spec 评审通过后）
2. 人工审核 spec.md 满意后跑 `/cc-nexs:approve-spec`
3. 再跑 `/cc-nexs:run`，全自动跑完所有 sprint 直到 COMPLETE
...
/cc-nexs:approve-spec   ← 唯一人工 gate 放行
