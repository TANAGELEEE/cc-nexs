...
function findProgressFiles(root, depth = 0) {
  ...
  for (const e of ents) {
    ...
    else if (e === 'progress.json' || (e === 'progress.md' && !existsSync(join(root, 'progress.json')))) out.push(full);
  }
  return out;
}
...
// ---- 2. authoritative progress state COMPLETE (scoped to this push) --------
...
if (touched === null) {
  console.error('[cc-nexs pre-merge] could not scope diff to this push — falling back to checking every progress file');
} else if (touched.length === 0) {
  console.error('[cc-nexs pre-merge] this push touches no progress file — skipping COMPLETE check');
}

for (const pf of pfsToCheck) {
  const text = readFileSync(pf, 'utf-8');
  const state = pf.endsWith('.json')
    ? (() => { try { return JSON.parse(text).state; } catch { return null; } })()
    : text.match(/current_state:\s*(\S+)/)?.[1];
  if (!state) continue;
  if (state !== 'COMPLETE') {
    console.error(`[cc-nexs pre-merge] ❌ ${pf}: state=${state}, expected COMPLETE`);
    fail = 1;
  }
}
