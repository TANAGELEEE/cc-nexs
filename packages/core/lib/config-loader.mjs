...
export function loadConfig({ projectRoot = process.cwd(), presetRoot = null } = {}) {
  ...
  // Organization/project rules stay outside the published preset. A project may
  // point at an overlay explicitly, or place one at .cc-nexs/overlay.{yml,json}.
  // Arrays replace the public defaults; objects merge recursively.
  const overlayPath = project.overlay_path
    ? resolve(projectRoot, project.overlay_path)
    : [
        resolve(projectRoot, '.cc-nexs/overlay.yml'),
        resolve(projectRoot, '.cc-nexs/overlay.json'),
      ].find(existsSync) || null;
  const overlay = overlayPath ? readStructured(overlayPath) : null;
  if (overlay) preset = deepMerge(preset || {}, overlay);
...
/**
 * Load the multi-repository workspace map. Repository paths are constrained to
 * the workspace root unless allow_external_paths is explicitly enabled.
 */
export function loadWorkspaceConfig({ projectRoot = process.cwd(), configPath = null } = {}) {
  const root = resolve(projectRoot);
  const candidates = configPath
    ? [resolve(root, configPath)]
    : [resolve(root, '.cc-nexs/workspace.yml'), resolve(root, '.cc-nexs/workspace.json')];
  const workspacePath = candidates.find(existsSync) || null;
  if (!workspacePath) return null;

  const raw = readStructured(workspacePath);
  if (raw?.version !== 1) throw new Error('[cc-nexs] workspace.version must be 1');
  if (!Array.isArray(raw.repositories) || raw.repositories.length === 0) {
    throw new Error('[cc-nexs] workspace.repositories must be a non-empty array');
  }

  const ids = new Set();
  const repositories = raw.repositories.map((repo) => {
    if (!/^[a-z][a-z0-9-]*$/.test(repo?.id || '')) {
      throw new Error(`[cc-nexs] invalid repository id: ${repo?.id || '<missing>'}`);
    }
    if (ids.has(repo.id)) throw new Error(`[cc-nexs] duplicate repository id: ${repo.id}`);
    ids.add(repo.id);
    if (typeof repo.path !== 'string' || !repo.path.trim()) {
      throw new Error(`[cc-nexs] repository ${repo.id} is missing path`);
    }
    const absolutePath = resolve(root, repo.path);
  ...
  if (raw.docs_repository && !ids.has(raw.docs_repository)) {
    throw new Error(`[cc-nexs] unknown docs_repository: ${raw.docs_repository}`);
  }
  ...
  return {
    ...
    workspacePath,
    projectRoot: root,
    worktree_root: resolve(root, raw.worktree_root || '.worktrees'),
    repositories,
  };
}
