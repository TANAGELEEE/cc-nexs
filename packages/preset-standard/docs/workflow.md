...
普通仓库维护、完整的既有需求和小范围文档修改不强制脑暴。显式 brainstorm 只补全 requirements.md，不增加逐节审批；Lean 的正式计划审批仍由 Gateway A 承接。

在批准范围内连续执行，沿用已有授权。常规实现选择自行解决；范围改变走当前模式的 gate。恢复会话时读取权威状态与当前 candidate，复用仍有效的证据，避免重派已完成角色。

每次只加载当前命令、当前模式 rule 和本次角色需要的材料。并行任务应有独立目标、明确路径所有权和实际收益；受状态机 barrier、共享仓库写入与运行时并发上限约束。Review 与黑盒验收的独立会话保持不变。

检查覆盖批准 AC 和受影响行为。已有检查通过后，只因新改动、失败或明确的新风险扩大验证；candidate 改变后仍按模式要求更新证据。输出遵循 `rules/output-discipline.md`，保留解析器需要的结论和全部有效 finding。
