...
description: /cc-nexs-minimal:review 的 Codex 镜像 skill。 当用户输入 "/cc-nexs-minimal:review"、"/cc-nexs-minimal:review ..."、"$cc-nexs-minimal-review" 或要求执行 cc-nexs review 流程时触发。 Invoke Reviewer subagent for spec / code / acceptance review.
---

# /cc-nexs-minimal:review for Codex

This skill is the Codex mirror for `/cc-nexs-minimal:review`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/review.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs-minimal:review` as the command arguments.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/review.md` are present in the original cc-nexs locations.
