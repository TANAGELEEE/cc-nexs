---
name: cc-nexs-init
description: /cc-nexs:init 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:init"、"/cc-nexs:init ..."、"$cc-nexs-init" 或要求执行 cc-nexs init 流程时触发。 Initialize progress.json v2 and create one isolated worktree per configured repository through Git Custodian.
---

# /cc-nexs:init for Codex

This skill is the Codex mirror for `/cc-nexs:init`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/init.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:init` as the command arguments.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/init.md` are present in the original cc-nexs locations.
