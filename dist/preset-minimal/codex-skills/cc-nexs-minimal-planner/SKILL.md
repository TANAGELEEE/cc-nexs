---
name: cc-nexs-minimal-planner
description: /cc-nexs-minimal:planner 的 Codex 镜像 skill。 当用户输入 "/cc-nexs-minimal:planner"、"/cc-nexs-minimal:planner ..."、"$cc-nexs-minimal-planner" 或要求执行 cc-nexs planner 流程时触发。 Invoke Planner subagent to draft or revise spec.md.
---

# /cc-nexs-minimal:planner for Codex

This skill is the Codex mirror for `/cc-nexs-minimal:planner`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/planner.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs-minimal:planner` as the command arguments.
...
5. Keep implementation and review in distinct native agent sessions. Provider-specific IDs are allowed only in private project/feature config; public preset defaults remain portable and inherit when unspecified.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/planner.md` are present in the original cc-nexs locations.
