...
// G2 (DEPLOY_GATE): blocks coding/testing/reviewer execution. Git mutation is
// independently restricted to Git Custodian by git-custodian-guard.mjs.
...
// Commands blocked during G2. Publishing/deployment is an explicit external
// release action, not a role-owned shell sequence.
const G2_FORBIDDEN = [
  /\bcodex\b/,
  /\bmvn\s+test\b/,
  /\bpnpm\s+test\b/,
...
function findProgressFiles(root, depth = 0, max = 4) {
  ...
  for (const e of entries) {
    ...
    } else if (e === 'progress.json' || (e === 'progress.md' && !existsSync(join(root, 'progress.json')))) {
      out.push(full);
    }
  }
  return out;
...
for (const pf of progressFiles) {
  ...
  let state = null;
  if (pf.endsWith('.json')) {
    try { state = JSON.parse(text).state; } catch { continue; }
  } else {
    state = text.match(/current_state:\s*(\S+)/)?.[1] || null;
  }
  if (!state) continue;
