...
```bash
codex "你是本项目的 QA。读 doc/<编号>/spec.md（重点：Sprint M<N> 对应的 AC-ID）和 doc/<编号>/api-doc.md。

append 到 doc/<编号>/test-cases.md 的 ## Sprint M<N> 章节。
...
```bash
codex "你是本项目的 QA。执行 doc/<编号>/test-cases.md 中 ## Sprint M<N> 下 auto 的 P0/P1 用例。

执行方式：
- API 测试：用 newman / curl / httpie 写脚本，放 doc/<编号>/qa-scripts/，调用真实接口
- 单元测试：跑 mvn test -Dtest=<类名>（仅黑盒视角看通过率，不读源码）
- E2E：用 Playwright 或同类，写在 e2e-smoke 仓里，跑后看结果

bug 落 doc/<编号>/bugs/BUG-<N>.md（必含可复现脚本，放 qa-scripts/BUG-<N>-repro.*）。

append 到 doc/<编号>/test-report.md 的 ## Sprint M<N> Round 1 章节。
...
```bash
codex "你是本项目的 QA。读 doc/<编号>/bugs/ 下 Sprint M<N> 相关且状态为 FIXED 的所有 BUG。
...
2. `find doc/<编号>/bugs/ -name 'BUG-*.md'` 统计 OPEN / FIXED / VERIFIED 数量
