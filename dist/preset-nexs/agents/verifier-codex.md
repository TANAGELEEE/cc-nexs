...
- doc/<编号>/api-doc.md
- doc/<编号>/deploy.md（如启动命令）

【任务 1：写测试用例】
追加到 doc/<编号>/test-cases.md 的 ## Sprint M1 章节。
要求：
...
- API 测试：写 newman / curl 脚本到 doc/<编号>/qa-scripts/
- 单元/集成：mvn test -Dtest=<类名>
- E2E：用 Playwright 或同类
- 发现 bug → 落到 doc/<编号>/bugs/BUG-<n>.md（必带可复现脚本到 qa-scripts/BUG-<n>-repro.*）

【任务 3：写报告】
append 到 doc/<编号>/test-report.md 的 ## Sprint M1 Round 1 章节。
...
【任务】
1. 读 doc/<编号>/bugs/ 下 Sprint M1 相关 + 状态 FIXED 的所有 BUG
...
append 到 doc/<编号>/test-report.md ## Sprint M1 回归 Round R 章节。
...
| 阶段 | 结论 | 下一步 |
|---|---|---|
| initial | 通过 | → ACCEPT（Reviewer） |
| initial | 阻塞 | → SPRINT_FIX（fix_per_bug 计数器在 BUG 修复时累加）|
| regression | 通过 | → ACCEPT（Reviewer） |
