---
name: cc-nexs-dev
description: /cc-nexs:dev 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:dev"、"/cc-nexs:dev ..."、"$cc-nexs-dev" 或要求执行 cc-nexs dev 流程时触发。 Tech Lead 编码入口。三种模式：feat（sprint 编码）/ fix（修 bug）/ doc（同步文档）。每次完成后 mvn compile + 中文字符串自检。
---

# /cc-nexs:dev for Codex

This skill is the Codex mirror for `/cc-nexs:dev`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/dev.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:dev` as the command arguments.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/dev.md` are present in the original cc-nexs locations.
