---
name: cc-nexs-review
description: /cc-nexs:review 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:review"、"/cc-nexs:review ..."、"$cc-nexs-review" 或要求执行 cc-nexs review 流程时触发。 fast 模式 Reviewer 角色入口。两种 target：spec 评审 / 代码评审 + 契约验收（合并）。通过 codex CLI 调用。
---

# /cc-nexs:review for Codex

This skill is the Codex mirror for `/cc-nexs:review`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/review.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:review` as the command arguments.
