...
description: /cc-nexs:approve-deploy 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:approve-deploy"、"/cc-nexs:approve-deploy ..."、"$cc-nexs-approve-deploy" 或要求执行 cc-nexs approve-deploy 流程时触发。 Approve the deploy gate (G2), marking g2_approved in progress.md then resuming the pipeline via /cc-nexs:run.
---
