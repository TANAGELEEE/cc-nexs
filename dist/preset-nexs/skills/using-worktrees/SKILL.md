...
- `git fetch` 失败（离线 / 无 remote）：fallback 用本地 HEAD 作为 base，打印 `⚠️ 无法 fetch origin，基于本地 HEAD 创建`
...
- 各 agent 仍在 worktree 内的 `all-docs/doc/<id>/` 下读写
