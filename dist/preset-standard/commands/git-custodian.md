...
- `create`: fetch the configured remote base, then create one same-named feature branch with no upstream and one isolated worktree per selected repository under `.worktrees/<id>-<slug>/<repo-id>/`. Never branch from the caller's current checkout. Pre-G1 implementation sync may request `recoverExisting` only to close a crash between worktree creation and progress persistence; recovery requires the exact registered path/branch, a clean tree, and a HEAD that is still an ancestor of the configured remote base. A partial path/branch pair or any implementation commit fails closed.
