...
description: /cc-nexs:approve-release 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-approve-release" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 Approve the exact Lean or Hotfix candidate verified in test before base-branch integration.
---

# /cc-nexs:approve-release for Codex

This explicit-only skill is the Codex mirror for `/cc-nexs:approve-release`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
