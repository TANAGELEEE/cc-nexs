---
name: cc-nexs-approve-release
description: /cc-nexs:approve-release 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:approve-release"、"/cc-nexs:approve-release ..."、"$cc-nexs-approve-release" 或要求执行 cc-nexs approve-release 流程时触发。 Approve the exact Lean or Hotfix candidate verified in test before base-branch integration.
---

# /cc-nexs:approve-release for Codex

This skill is the Codex mirror for `/cc-nexs:approve-release`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/approve-release.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:approve-release` as the command arguments.
...
```text
node <resolved-cli-path> approve-release <feature-id>
```

Never execute `/cc-nexs:approve-release` as a shell path and never edit `progress.json` or `progress.md` directly. After the control program succeeds, continue the current runtime's run workflow.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/approve-release.md` are present in the original cc-nexs locations.
