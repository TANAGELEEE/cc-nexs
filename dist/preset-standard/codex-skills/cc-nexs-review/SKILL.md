---
name: cc-nexs-review
description: /cc-nexs:review 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-review" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 fast 模式 Reviewer 入口。spec、每轮发布前代码评审、部署后最终契约验收三者保持独立调用。
---

# /cc-nexs:review for Codex

This explicit-only skill is the Codex mirror for `/cc-nexs:review`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
