...
description: /cc-nexs:brainstorm 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:brainstorm"、"/cc-nexs:brainstorm ..."、"$cc-nexs-brainstorm" 或要求执行 cc-nexs brainstorm 流程时触发。 Brainstorming 入口。用 Socratic 对话把 PM 一句话诉求展开成结构化 requirements.md，作为 Planner 的输入。在 /cc-nexs:init 之后、/cc-nexs:run 之前调用。
---

# /cc-nexs:brainstorm for Codex

This skill is the Codex mirror for `/cc-nexs:brainstorm`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/brainstorm.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:brainstorm` as the command arguments.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/brainstorm.md` are present in the original cc-nexs locations.
