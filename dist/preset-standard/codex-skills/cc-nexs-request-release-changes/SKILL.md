---
name: cc-nexs-request-release-changes
description: /cc-nexs:request-release-changes 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:request-release-changes"、"/cc-nexs:request-release-changes ..."、"$cc-nexs-request-release-changes" 或要求执行 cc-nexs request-release-changes 流程时触发。 Record Gateway B feedback and route evidence-only, implementation, or scope changes safely.
---

# /cc-nexs:request-release-changes for Codex

This skill is the Codex mirror for `/cc-nexs:request-release-changes`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/request-release-changes.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:request-release-changes` as the command arguments.

## Deterministic Gateway B Change Control
...
The command is complete only when the artifact, state, and summary expected by `../../commands/request-release-changes.md` are present in the original cc-nexs locations.
