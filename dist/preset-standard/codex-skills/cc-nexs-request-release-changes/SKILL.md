...
description: /cc-nexs:request-release-changes 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-request-release-changes" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 Record Gateway B feedback and route evidence-only, implementation, or scope changes safely.
---

# /cc-nexs:request-release-changes for Codex

This explicit-only skill is the Codex mirror for `/cc-nexs:request-release-changes`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
