---
name: cc-nexs-release-base
description: /cc-nexs:release-base 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:release-base"、"/cc-nexs:release-base ..."、"$cc-nexs-release-base" 或要求执行 cc-nexs release-base 流程时触发。 Integrate exact approved Lean/Hotfix candidates into configured base branches and clean worktrees after proof.
---

# /cc-nexs:release-base for Codex

This skill is the Codex mirror for `/cc-nexs:release-base`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/release-base.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:release-base` as the command arguments.
...
```text
node <resolved-cli-path> release-base <feature-id>
```
...
The command is complete only when the artifact, state, and summary expected by `../../commands/release-base.md` are present in the original cc-nexs locations.
