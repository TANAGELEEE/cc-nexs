...
description: /cc-nexs:release-base 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-release-base" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 Integrate exact approved Lean/Hotfix candidates into configured base branches and clean worktrees after proof.
---

# /cc-nexs:release-base for Codex

This explicit-only skill is the Codex mirror for `/cc-nexs:release-base`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
