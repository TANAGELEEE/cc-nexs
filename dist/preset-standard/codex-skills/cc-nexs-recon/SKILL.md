---
name: cc-nexs-recon
description: /cc-nexs:recon 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:recon"、"/cc-nexs:recon ..."、"$cc-nexs-recon" 或要求执行 cc-nexs recon 流程时触发。 Repo Scout 角色入口。读 src/ 把现存同类配置/Service/页面/API 归档成 repo-context.md，作为 Planner 的现状清单输入。在 /cc-nexs:brainstorm 之后、Planner 之前调用（编排器 RECON 阶段会自动跑）。
---

# /cc-nexs:recon for Codex

This skill is the Codex mirror for `/cc-nexs:recon`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/recon.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:recon` as the command arguments.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/recon.md` are present in the original cc-nexs locations.
