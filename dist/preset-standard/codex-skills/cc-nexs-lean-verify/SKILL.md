---
name: cc-nexs-lean-verify
description: /cc-nexs:lean-verify 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:lean-verify"、"/cc-nexs:lean-verify ..."、"$cc-nexs-lean-verify" 或要求执行 cc-nexs lean-verify 流程时触发。 Lean test 环境黑盒验收，直接把 AC 证据汇总到 plan.md。
---

# /cc-nexs:lean-verify for Codex

This skill is the Codex mirror for `/cc-nexs:lean-verify`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/lean-verify.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:lean-verify` as the command arguments.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/lean-verify.md` are present in the original cc-nexs locations.
