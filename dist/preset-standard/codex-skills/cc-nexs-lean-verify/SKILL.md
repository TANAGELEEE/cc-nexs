...
description: /cc-nexs:lean-verify 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-lean-verify" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 Lean test 环境黑盒验收，直接把 AC 证据汇总到 plan.md。
---

# /cc-nexs:lean-verify for Codex

This explicit-only skill is the Codex mirror for `/cc-nexs:lean-verify`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
