---
name: cc-nexs-hotfix
description: /cc-nexs:hotfix 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-hotfix" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 独立 Hotfix mini-Lean：latest-base worktree、本地验证、一次集中 Review、test 黑盒验收、人工 base 门禁。
---

# /cc-nexs:hotfix for Codex

This explicit-only skill is the Codex mirror for `/cc-nexs:hotfix`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
