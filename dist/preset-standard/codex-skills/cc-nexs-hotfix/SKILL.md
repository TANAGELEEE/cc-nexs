---
name: cc-nexs-hotfix
description: /cc-nexs:hotfix 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:hotfix"、"/cc-nexs:hotfix ..."、"$cc-nexs-hotfix" 或要求执行 cc-nexs hotfix 流程时触发。 独立 Hotfix mini-Lean：latest-base worktree、本地验证、一次集中 Review、test 黑盒验收、人工 base 门禁。
---

# /cc-nexs:hotfix for Codex

This skill is the Codex mirror for `/cc-nexs:hotfix`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/hotfix.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:hotfix` as the command arguments.

## Deterministic Hotfix Controls

Resolve `../../lib/cc-nexs-cli.mjs` relative to this SKILL.md. Bind the completed hotfix scope with:
...
The command is complete only when the artifact, state, and summary expected by `../../commands/hotfix.md` are present in the original cc-nexs locations.
