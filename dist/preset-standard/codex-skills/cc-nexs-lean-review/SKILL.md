...
description: /cc-nexs:lean-review 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-lean-review" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 Lean 一次集中 Review 与最多一次 delta 闭环检查。
---

# /cc-nexs:lean-review for Codex

This explicit-only skill is the Codex mirror for `/cc-nexs:lean-review`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
