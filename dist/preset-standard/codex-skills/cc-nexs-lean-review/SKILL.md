...
description: /cc-nexs:lean-review 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:lean-review"、"/cc-nexs:lean-review ..."、"$cc-nexs-lean-review" 或要求执行 cc-nexs lean-review 流程时触发。 Lean 一次集中 Review 与最多一次 delta 闭环检查。
---

# /cc-nexs:lean-review for Codex

This skill is the Codex mirror for `/cc-nexs:lean-review`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/lean-review.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:lean-review` as the command arguments.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/lean-review.md` are present in the original cc-nexs locations.
