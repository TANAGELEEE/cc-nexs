---
name: cc-nexs-planner
description: /cc-nexs:planner 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:planner"、"/cc-nexs:planner ..."、"$cc-nexs-planner" 或要求执行 cc-nexs planner 流程时触发。 Planner 角色入口。展开业务需求为 spec.md，含验收契约 AC 表 + Sprint 切片。可选 --revise 模式按 SA 反馈修订。
---

# /cc-nexs:planner for Codex

This skill is the Codex mirror for `/cc-nexs:planner`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/planner.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:planner` as the command arguments.
