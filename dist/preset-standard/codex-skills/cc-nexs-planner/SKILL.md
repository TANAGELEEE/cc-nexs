...
description: /cc-nexs:planner 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-planner" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 Planner 角色入口。展开业务需求为 spec.md，含验收契约 AC 表 + Sprint 切片。可选 --revise 模式按 SA 反馈修订。
---

# /cc-nexs:planner for Codex

This explicit-only skill is the Codex mirror for `/cc-nexs:planner`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
