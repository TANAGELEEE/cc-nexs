...
This skill is the Codex mirror for `/cc-nexs:evaluator`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/evaluator.md` are present in the original cc-nexs locations.
