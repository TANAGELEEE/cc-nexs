...
description: /cc-nexs:evaluator 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-evaluator" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 Evaluator 契约验收入口。新流程默认 final，仅在完整需求发布并通过最终 QA 后全量验收；sprint 仅供 legacy per_sprint。
---
