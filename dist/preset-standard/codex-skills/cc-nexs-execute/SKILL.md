---
name: cc-nexs-execute
description: /cc-nexs:execute 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:execute"、"/cc-nexs:execute ..."、"$cc-nexs-execute" 或要求执行 cc-nexs execute 流程时触发。 Lean 实现入口。按 plan 任务和路径所有权并行实现，不负责 Git mutation。
---

# /cc-nexs:execute for Codex

This skill is the Codex mirror for `/cc-nexs:execute`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/execute.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:execute` as the command arguments.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/execute.md` are present in the original cc-nexs locations.
