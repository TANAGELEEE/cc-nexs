...
description: /cc-nexs:execute 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-execute" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 Lean 实现入口。按 plan 任务和路径所有权并行实现，不负责 Git mutation。
---

# /cc-nexs:execute for Codex

This explicit-only skill is the Codex mirror for `/cc-nexs:execute`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
