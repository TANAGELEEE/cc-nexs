...
description: /cc-nexs:fullstack 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-fullstack" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 fast 模式 Fullstack 入口。负责 spec、实现、文档和评审修订；发布后 BUG 只修到 FIXED，部署回归由 Verifier 完成。
---

# /cc-nexs:fullstack for Codex

This explicit-only skill is the Codex mirror for `/cc-nexs:fullstack`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/fullstack.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:fullstack` as the command arguments.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/fullstack.md` are present in the original cc-nexs locations.
