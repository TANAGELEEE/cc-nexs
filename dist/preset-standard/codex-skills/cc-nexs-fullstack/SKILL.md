...
description: /cc-nexs:fullstack 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-fullstack" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 fast 模式 Fullstack 入口。spec 后按批准的 repository assignment 并行实现，再由单一 owner 同步文档。
---

# /cc-nexs:fullstack for Codex

This explicit-only skill is a thin Codex runtime adapter for `/cc-nexs:fullstack`.
