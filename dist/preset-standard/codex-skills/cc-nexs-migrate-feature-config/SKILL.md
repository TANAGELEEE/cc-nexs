---
name: cc-nexs-migrate-feature-config
description: /cc-nexs:migrate-feature-config 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-migrate-feature-config" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 Safely migrate an existing Lean/Hotfix config.json to automatic risk-based model routing without removing explicit overrides.
---

# /cc-nexs:migrate-feature-config for Codex

This explicit-only skill is the Codex mirror for `/cc-nexs:migrate-feature-config`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/migrate-feature-config.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:migrate-feature-config` as the command arguments.

## Deterministic Lean Control

Resolve `../../lib/cc-nexs-cli.mjs` relative to this SKILL.md and execute:

```text
node <resolved-cli-path> migrate-feature-config <feature-id> [--dry-run] [--bind-plan-risk] [--progress <path>]
```
...
The command is complete only when the artifact, state, and summary expected by `../../commands/migrate-feature-config.md` are present in the original cc-nexs locations.
