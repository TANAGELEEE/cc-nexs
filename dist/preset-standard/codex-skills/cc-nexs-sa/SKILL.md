---
name: cc-nexs-sa
description: /cc-nexs:sa 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-sa" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 SA 评审入口。支持 spec、cases、单 Sprint code、跨 Sprint integration 和发布后 fix code 评审。
---

# /cc-nexs:sa for Codex

This explicit-only skill is the Codex mirror for `/cc-nexs:sa`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/sa.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:sa` as the command arguments.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/sa.md` are present in the original cc-nexs locations.
