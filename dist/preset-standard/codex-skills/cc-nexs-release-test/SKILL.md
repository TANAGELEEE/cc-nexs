...
This explicit-only skill is the Codex mirror for `/cc-nexs:release-test`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
...
Complete the runtime/browser capability preflight from the authoritative command before any remote mutation. Then resolve `../../lib/cc-nexs-cli.mjs` relative to this SKILL.md and execute:
