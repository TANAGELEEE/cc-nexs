---
name: cc-nexs-qa
description: /cc-nexs:qa 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-qa" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 QA 黑盒测试入口。Sprint 阶段写/评用例；完整需求发布到 test 后执行 final 或 final-regression。禁读 src/。
---

# /cc-nexs:qa for Codex

This explicit-only skill is the Codex mirror for `/cc-nexs:qa`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/qa.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:qa` as the command arguments.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/qa.md` are present in the original cc-nexs locations.
