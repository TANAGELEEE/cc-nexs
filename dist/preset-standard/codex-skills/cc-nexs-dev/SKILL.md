---
name: cc-nexs-dev
description: /cc-nexs:dev 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-dev" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 Tech Lead 编码入口。支持 Sprint 开发、文档同步、评审修订、跨 Sprint 集成修订和发布后 BUG 修复。
---

# /cc-nexs:dev for Codex

This explicit-only skill is the Codex mirror for `/cc-nexs:dev`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
