---
name: cc-nexs-dev
description: /cc-nexs:dev 的 Codex 镜像 skill。 仅当用户显式输入 "$cc-nexs-dev" 或在界面中选择该 skill 时使用；不得因普通自然语言请求自动触发。 Tech Lead 编码入口。Sprint 实现可按已批准 repository assignment 并行；文档、评审修订与修复保持边界隔离。
---

# /cc-nexs:dev for Codex

This explicit-only skill is a thin Codex runtime adapter for `/cc-nexs:dev`.
