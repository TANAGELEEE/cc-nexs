---
name: cc-nexs-plan
description: /cc-nexs:plan 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:plan"、"/cc-nexs:plan ..."、"$cc-nexs-plan" 或要求执行 cc-nexs plan 流程时触发。 Lean 默认模式计划入口。并行只读调研，生成唯一 requirements.md 与 plan.md，并停在 Gateway A。
---

# /cc-nexs:plan for Codex

This skill is the Codex mirror for `/cc-nexs:plan`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
