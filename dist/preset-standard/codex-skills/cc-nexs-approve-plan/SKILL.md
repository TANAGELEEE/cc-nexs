---
name: cc-nexs-approve-plan
description: /cc-nexs:approve-plan 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:approve-plan"、"/cc-nexs:approve-plan ..."、"$cc-nexs-approve-plan" 或要求执行 cc-nexs approve-plan 流程时触发。 Approve the Lean requirements and plan scope, binding the gate to their hashes.
---

# /cc-nexs:approve-plan for Codex

This skill is the Codex mirror for `/cc-nexs:approve-plan`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.

## Authoritative Command

Read and follow `../../commands/approve-plan.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:approve-plan` as the command arguments.
...
```text
node <resolved-cli-path> approve-plan <feature-id>
```

Never execute `/cc-nexs:approve-plan` as a shell path and never edit `progress.json` or `progress.md` directly. After the control program succeeds, continue the current runtime's run workflow.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/approve-plan.md` are present in the original cc-nexs locations.
