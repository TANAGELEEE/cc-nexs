...
description: /cc-nexs:verify-local 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:verify-local"、"/cc-nexs:verify-local ..."、"$cc-nexs-verify-local" 或要求执行 cc-nexs verify-local 流程时触发。 Run deterministic Lean/Hotfix local build/start/smoke/E2E and bind evidence to exact candidate commits.
---

# /cc-nexs:verify-local for Codex

This skill is the Codex mirror for `/cc-nexs:verify-local`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
...
```text
node <resolved-cli-path> verify-local <feature-id>
```
