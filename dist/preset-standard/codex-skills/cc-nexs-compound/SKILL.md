...
description: /cc-nexs:compound 的 Codex 镜像 skill。 显式调用 $cc-nexs-compound 时使用。 Compound 角色入口。读完成需求的 all-docs/doc/<id>/* 把\"非显然教训\"沉淀到 docs/solutions/<topic>.md，下次同类需求 Repo Scout 自动接入。旁路命令，不进状态机。
---
