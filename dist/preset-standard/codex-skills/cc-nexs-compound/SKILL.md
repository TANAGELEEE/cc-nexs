...
description: /cc-nexs:compound 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:compound"、"/cc-nexs:compound ..."、"$cc-nexs-compound" 或要求执行 cc-nexs compound 流程时触发。 Compound 角色入口。读完成需求的 all-docs/doc/<id>/* 把"非显然教训"沉淀到 docs/solutions/<topic>.md，下次同类需求 Repo Scout 自动接入。旁路命令，不进状态机。
---

# /cc-nexs:compound for Codex

This skill is the Codex mirror for `/cc-nexs:compound`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
