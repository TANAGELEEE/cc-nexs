---
name: cc-nexs-init
description: /cc-nexs:init 的 Codex 镜像 skill。 当用户输入 "/cc-nexs:init"、"/cc-nexs:init ..."、"$cc-nexs-init" 或要求执行 cc-nexs init 流程时触发。 用模板初始化新需求目录。自动按 all-docs/doc/ 下已有编号续号，自动从需求描述生成 slug。默认在 .worktrees/<id>-<slug>/ 创建独立 git worktree，多需求可并行。
---

# /cc-nexs:init for Codex

This skill is the Codex mirror for `/cc-nexs:init`. It exists so the Codex plugin can preserve the same command surface, workflow semantics, document write locations, and lean / full / fast / hotfix behavior as the Claude Code plugin.
