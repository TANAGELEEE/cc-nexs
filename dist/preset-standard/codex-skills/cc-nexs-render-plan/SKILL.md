...
description: /cc-nexs:render-plan 的 Codex 镜像 skill。 显式调用 $cc-nexs-render-plan 时使用。 Render the Lean plan Markdown as a human-friendly temporary HTML page.
---

# /cc-nexs:render-plan for Codex

This explicit-only skill is a thin Codex runtime adapter for `/cc-nexs:render-plan`.

## Authoritative Command

Read and follow `../../commands/render-plan.md` as the single source of truth for this command. Treat the user's original message after `/cc-nexs:render-plan` as the command arguments.
...
The command is complete only when the artifact, state, and summary expected by `../../commands/render-plan.md` are present in the original cc-nexs locations.
